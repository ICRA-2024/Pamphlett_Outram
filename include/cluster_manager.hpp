...
// template<typename T>
// inline void pcl2teaser(const pcl::PointCloud<T> &pcl_raw, teaser::PointCloud
// &cloud) {
//     cloud.clear();
...
// template<typename T>
// inline void eigen2pcl(const Eigen::Matrix<double, 3, Eigen::Dynamic> &src,
...
// template<typename T>
// inline void pcl2eigen(const pcl::PointCloud<T> &pcl_raw,
// Eigen::Matrix<double, 3, Eigen::Dynamic> &cloud) {
//     int N = pcl_raw.points.size();
//     cloud.resize(3, N);
//     for (int i = 0; i < N; ++i) {
//         cloud.col(i) << pcl_raw.points[i].x, pcl_raw.points[i].y,
//         pcl_raw.points[i].z;
//     }
// }
...
public:
    ...
    bool selectSemanticPoints(
            pcl::PointCloud<PointL>::Ptr input_src_sem_cloud,
    ...
    void dbscanSeg(pcl::PointCloud<PointL>::Ptr input_sem_cloud) {
        ...
        tree->setInputCloud(selected_points_);

        std::vector<pcl::PointIndices> cluster_indices;
        pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec;
        ec.setClusterTolerance(params_.clusterTolerance);  // 设置欧式距离阈值
        ec.setMinClusterSize(params_.minClusterSize);  // 设置最小聚类点数
        ec.setMaxClusterSize(params_.maxClusterSize);  // 设置最大聚类点数
        ...
        // // regions_[0] = 14; regions_[1] = 14; regions_[2] = 14; regions_[3]
        // = 15; regions_[4] = 14; regions_[0] = 4; regions_[1] = 5; regions_[2]
        // = 4; regions_[3] = 5; regions_[4] = 4; regions_[5] = 5; regions_[6] =
        // 5; regions_[7] = 4; regions_[8] = 5; regions_[9] = 4; regions_[10]=
        // 5; regions_[11]= 5; regions_[12]= 4; regions_[13]= 20;
        ...
        //         float d2 = selected_points_->points[i].x *
        //         selected_points_->points[i].x +
        //                    selected_points_->points[i].y *
        //                    selected_points_->points[i].y +
        //                    selected_points_->points[i].z *
        //                    selected_points_->points[i].z;
        ...
        //         pcl_pc_in->points[(*pc_indices)[i]].z; if(d2 > range * range
        ...
        //             for(int j = last_clusters_begin; j < last_clusters_end;
        //             j++) {
        ...
        //                     clusters_[j]->points.size()-1, K, k_indices,
        //                     k_sqr_distances) > 0) {
        ...
        //                             last_clusters_end--;
        //                             //std::cerr << "k-merging: clusters_ " <<
        //                             j << " is merged" << std::endl;
        //                         }
        //                     }
        //                 }
        //             }
        ...
        //                 if(std::max(std::min((double)j_max[0],
        //                 (double)k_max[0]) - std::max((double)j_min[0],
        //                 (double)k_min[0]), 0.0)
        //                    * std::max(std::min((double)j_max[1],
        //                    (double)k_max[1]) - std::max((double)j_min[1],
        //                    (double)k_min[1]), 0.0) > z_merging_threshold_
        //                 //    || std::pow(std::min((double)j_max[0],
        //                 (double)k_max[0]) - std::max((double)j_min[0],
        //                 (double)k_min[0]), 2)
        //                 //    + std::pow(std::min((double)j_max[1],
        //                 (double)k_max[1]) - std::max((double)j_min[1],
        //                 (double)k_min[1]), 2)
        //                 //    + std::pow(std::min((double)j_max[2],
        //                 (double)k_max[2]) - std::max((double)j_min[2],
        //                 (double)k_min[2]), 2) < 5
        //                    ) {
        //                     *clusters_[j] += *clusters_[k];
        //                     clusters_.erase(clusters_.begin() + k);
        //                     //std::cerr << "z-merging: clusters_ " << k << "
        //                     is merged into " << j << std::endl;
        //                 }
        //             }
        //         }
        //         /*****************************/
    ...
    /**
     * @brief Compute centroid and covariance matrix for every cluster center
     *
     * @param cluster_centroid output cluster centers with semantic
     * label and normal
     * @param covariances output vactor containing covariances of each cluster
     */
    void computeCloudwNormal(
            pcl::PointCloud<pcl::PointXYZLNormal>::Ptr cluster_centroid,
        ...
        for (int i = 0; i < cluster_size; ++i) {
            ...
            for (int j = 0; j < clusters_[i]->points.size(); j++) {
                // neighbors.col(j) =
                // selected_points_->at(index_vec[j]).getVector3fMap().cast<double>();
                neighbors.col(j) =
                        clusters_[i]->at(j).getVector3fMap().cast<double>();
            }
            ...
            normals << evecs.real()(0, evalsMin), evecs.real()(1, evalsMin),
                    evecs.real()(2, evalsMin);

            pcl::PointXYZLNormal p_temp;
            p_temp.x = (float)center(0);
            p_temp.y = (float)center(1);
            p_temp.z = (float)center(2);
            p_temp.label = params_.semanticLabel;
            p_temp.normal_x = normals[0];
            p_temp.normal_y = normals[1];
            p_temp.normal_z = normals[2];
    ...
    void computeCentroidAndCov(
        ...
        for (int i = 0; i < cluster_size; ++i) {
            ...
            // compute centroid coordinate
            Eigen::Matrix<double, 3, 1> center = neighbors.rowwise().mean();
            PointType p_temp((float)center(0), (float)center(1),
                             (float)center(2));
            cluster_centroid->push_back(p_temp);
            // substract mean
            neighbors.colwise() -= center;
    ...
    void computeCov(pcl::PointCloud<PointType>::Ptr original_pt_cloud,
        ...
        for (size_t i = 0; i < index.size(); ++i) {
            int cur_idx = index[i];
            neighbors.col(i) = original_pt_cloud->at(cur_idx)
                                       .getVector3fMap()
                                       .cast<double>();
        }
        ...
        Eigen::Matrix<double, 3, 1> center = neighbors.rowwise().mean();
        // PointType p_temp((float)center(0), (float)center(1),
        // (float)center(2)); cluster_centroid->push_back(p_temp); substract
        // mean
        neighbors.colwise() -= center;
        // compute cov 3*3
    ...
    bool fpfh_feature_extraction(
        ...
        corr_ = matcher.calculateCorrespondences(
                src_cloud_, tgt_cloud_, *src_descriptors_, *tgt_descriptors_,
                true, true, true, 0.95);
        ...
        for (size_t i = 0; i < corr_.size(); ++i) {
            ...
            int size_src = src_tree.radiusSearch(temp_src, 1,
                                                 src_neighbor_index, src_dist);
            computeCov(srcFeat, src_neighbor_index, covariances_src_);
            int size_tgt = tgt_tree.radiusSearch(temp_tgt, 1,
                                                 tgt_neighbor_index, tgt_dist);
