...
public:
    ...
    struct Params {
        ...
        Params() {
            ...
            // teaser_params.rotation_tim_graph =
            //         TEASER::INLIER_GRAPH_FORMULATION::COMPLETE;
        }
    };
...
public:
    ...
    void solve(const Eigen::MatrixX3d &src,
               const Eigen::MatrixX3d &tgt) override;
    ...
    /**
     * @brief prune correspondence built by STD and solve the relative transform
     *
     * @param src_points indexed points in the current pc
     * @param tgt_points indexed points in the reference map
     */
    void solveSTDcorres(const Eigen::Matrix3Xd src_points,
                        const Eigen::Matrix3Xd tgt_points);

    /**
     * @brief prune correspondence built by STD w/ distribution-based cosistency
     * checks
     *
     * @param src_points indexed points in the current pc
     * @param tgt_points indexed points in the reference map
     * @param src_covariances corresponding cov mat of src_points
     * @param tgt_covariances corresponding cov mat of tgt_points
     */
    void solveSTDCovCorres(
    ...
    /** \brief solve cloud-wise transformation using multiple semantic
     * clusters with cov mat outlier pruning \param[in] src_cloud_vec
     * vector containing source clouds of different semantics \param[in]
     * tgt_cloud_vec vector containing target clouds of different
     * semantics \param[in] src_covariances vector of eigen matrix
     * containing cov mats of each point in src cloud \param[in]
     * tgt_covariances vector of eigen matrix containing cov mats of
    ...
    void setInliers(const Eigen::Matrix<double, 3, Eigen::Dynamic> &raw,
        ...
        for (const int idx : idx_inliers) {
            inliers.push_back(
                    pcl::PointXYZ(raw(0, idx), raw(1, idx), raw(2, idx)));
        }
    }
